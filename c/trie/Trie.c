...
int trie_delete(Trie *trie, char *elem) {
    ...
    if (cursor->num_of_worlds > 0) {
        cursor->num_of_worlds--;
        if (is_leaf(cursor) && cursor->num_of_worlds == 0) {
...
int trie_search(Trie *trie, char *elem) {
    ...
    if (cursor->num_of_worlds > 0) return 1;
    return 0;
}
...
void trie_print(Trie *trie) {
    for (int i = 0; i < TRIE_NUM_OF_CHARACTER; i++) {
        if (trie->children[i] != NULL) {
            printf("Node: %c with %d worlds\n", trie_mapping_int(i), trie->children[i]->num_of_worlds);
